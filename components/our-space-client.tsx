...
import { PersonalPanel } from "@/components/our-space/personal-panel";
import {
  MobileSpaceTabs,
  PeriodControls,
  PeriodPickerButton,
...
export function OurSpaceClient({
  ...
  return (
    <main className="min-h-svh overflow-x-clip bg-bg mobile-native-shell">
      <SpaceHero
      ...
      <section className="container-page pb-[calc(env(safe-area-inset-bottom)+10rem)] pt-4 sm:py-8">
        ...
        <div key={activeSection} className="mobile-tab-panel-in sm:contents">
          {activeSection === "notes" ? (
            <NotesPanel
